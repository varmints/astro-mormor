import { type ChangeEvent, type FormEvent, useState, useRef, useLayoutEffect } from 'react'
...
export default function ContactForm(props: Props) {
	...
	useLayoutEffect(() => {
		setInitialHeight(parseInt(getComputedStyle(textAreaRef.current!).getPropertyValue('height')))
		setInitialBorderWidth(
			parseInt(getComputedStyle(textAreaRef.current!).getPropertyValue('border-bottom-width'))
		)
	}, [])
		...
		<form onSubmit={submitForm} className="mx-auto">
			<div className="divide-y">
				<div className="space-y-10 leading-7">
					{message && (
						<div className={`mb-4 w-full p-4 text-white ${message.class}`}>{message.text}</div>
					)}
					...
					<div className="flex justify-center">
						<Turnstile sitekey={captchaKey} onVerify={updateCaptchaToken} theme="dark" />
						<button
							disabled={submitting}
